 ...
 * ``json.dumps(..., ensure_ascii=False, separators=(',', ':'), sort_keys=True, allow_nan=False)``.
 *
 * Shared by the browser playground and Node parity tests.
 */

function compareUnicodeKeys(a, b) {
...
function serializeJsonString(s) {
  let out = '"';
  for (const ch of s) {
...
/** Match Python ``json`` exponent form (e.g. ``1e-05``, ``9e-05``). */
function formatExponentialPython(n) {
  let s = n.toExponential();
  const neg = s.startsWith("-");
  if (neg) s = s.slice(1);
  s = s.replace(/e([+-])(\d+)$/i, (_, sign, dig) => {
...
function serializeJsonNumber(n) {
...
function serializeJsonValue(v) {
  ...
  if (typeof v === "object") {
    const keys = Object.keys(v).sort(compareUnicodeKeys);
    return (
      "{" +
      keys.map((k) => serializeJsonString(k) + ":" + serializeJsonValue(v[k])).join(",") +
      "}"
    );
  }
...
export function canonicalJsonStr(value) {
