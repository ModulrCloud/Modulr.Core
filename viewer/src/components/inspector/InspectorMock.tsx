...
/**
 * Operator-focused module metrics and charts — lives at `/inspector`, not the public home.
 */
export function InspectorMock() {
  const [moduleId, setModuleId] = useState(DEFAULT_MODULE_ID);
  const metrics = useMemo(() => getMockMetrics(moduleId), [moduleId]);
  const userTypeSlices = useMemo(() => getMockUserTypeMix(moduleId), [moduleId]);
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="modulr-text-muted mt-2 max-w-2xl text-sm leading-relaxed">
                Module-level metrics and activity for operators and validators. Core version lives in
                the header. Numbers below are{" "}
      ...
      <section aria-labelledby="news-heading" className="w-full">
        <GlassPanel className="p-6 sm:p-8">
          ...
          <ul className="mt-6 space-y-4">
            {[
              {
                title: "Inspector on its own route",
                date: "Today",
                body: "The welcome home is for everyone; Inspector stays here for module-level mock metrics.",
              },
              {
