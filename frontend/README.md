...
With Core running (e.g. `modulr-core --config dev.toml` on port **8000**), the header loads **`GET /version`** for the live `v…` label. **Methods** wires **`get_protocol_version`** and **`lookup_module`** to the real **`POST /message`** path (canonical JSON + Ed25519, same rules as the dev playground). Core **`dev_mode`** enables CORS for **`http://localhost:3000`** and **`http://127.0.0.1:3000`**; override with env **`MODULR_CORE_CORS_ORIGINS`** (comma-separated).
