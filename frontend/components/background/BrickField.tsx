 ...
 */
export function BrickField({ visible, animate = true, colorMode }: Props) {
  ...
  hudPaletteRef.current = (p: BrickPalette) => {
    const rgb = brickFillRgb(p, modeRef.current === "dark", 6);
    setHudRgb(rgb);
  };
  ...
  useEffect(() => {
    ...
    function triggerGameOver(reason: 'miss' | 'brick') {
      ...
      setLoseOverlay({
        open: true,
        rgb: brickFillRgb(brickPalette, isDark(), 6),
        reason,
      });
    }
  ...
  const hudStroke = strokeStyleForFill(hudRgb, colorMode === "dark");
